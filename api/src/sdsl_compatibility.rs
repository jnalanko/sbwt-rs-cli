...
pub fn load_sdsl_int_vector(input: &mut impl std::io::Read) -> std::io::Result<simple_sds_sbwt::int_vector::IntVector> {
...
mod tests {
    ...
    fn test_load_sdsl_int_vector(){
        ...
        let v = load_sdsl_int_vector(&mut std::io::Cursor::new(&data)).unwrap();
        assert!(v.len() == 20);
        assert!(v.width() == 5);
