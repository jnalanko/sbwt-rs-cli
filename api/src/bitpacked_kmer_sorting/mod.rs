...
/// Build using bitpacked k-mer sorting. See [crate::builder] for a wrapper with a more 
...
mod tests {
    ...
    fn disk_vs_mem_parallel_differential() {
        use crate::builder::{BitPackedKmerSortingDisk, BitPackedKmerSortingMem};
        ...
        for &k in &[3_usize, 8, 31] {
            ...
            for &n_threads in &[1_usize, 2, 3, 8] {
                let (mem_sbwt, mem_lcs) = BitPackedKmerSortingMem::new_from_slices(&[seq.as_slice()], k)
                    .k(k).n_threads(n_threads).build_lcs(true)
                    .run();

                let (disk_sbwt, disk_lcs) = BitPackedKmerSortingDisk::new_from_slices(&[seq.as_slice()], k)
                    .k(k).n_threads(n_threads).build_lcs(true)
                    .run();
    ...
    fn disk_vs_mem_suffix_group_boundary_stress() {
        ...
        for &n_threads in &[1_usize, 2, 4, 8, 16] {
            let (mem_sbwt, mem_lcs) = BitPackedKmerSortingMem::new_from_slices(&seq_refs, k)
                .k(k).n_threads(n_threads).build_lcs(true)
                .run();

            let (disk_sbwt, disk_lcs) = BitPackedKmerSortingDisk::new_from_slices(&seq_refs, k)
                .k(k).n_threads(n_threads).build_lcs(true)
                .run();
