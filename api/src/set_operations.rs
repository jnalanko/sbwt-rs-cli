...
mod tests {
    ...
    use crate::{BitPackedKmerSortingDisk, BitPackedKmerSortingMem, SbwtIndex, SubsetMatrix, SubsetSeq};
...
/// All distinct k-mers found in `seq`.
    ...
    fn test_intersect_orphaned_branch_dummy() {
        ...
        isec_kmers.sort();
        let (sbwt_true, _) = BitPackedKmerSortingMem::new_from_vecs(&isec_kmers, k).run();
